                  ...
                  PaddleDetection::ObjectDetector *det)
{
  ...
  int codec = cv::VideoWriter::fourcc('m', 'p', '4', 'v'); //增加的
      // Create VideoWriter for output
...
int main(int argc, char **argv)
{
  ...
  if (FLAGS_model_dir.empty() || (FLAGS_image_file.empty() && FLAGS_image_dir.empty() && FLAGS_video_file.empty()))
  {
    std::cout << "Usage: ./main --model_dir=/PATH/TO/INFERENCE_MODEL/ "
              << "--image_file=/PATH/TO/INPUT/IMAGE/" << std::endl;
    return -1;
  }
  if (!(FLAGS_run_mode == "fluid" || FLAGS_run_mode == "trt_fp32" || FLAGS_run_mode == "trt_fp16" || FLAGS_run_mode == "trt_int8"))
  {
